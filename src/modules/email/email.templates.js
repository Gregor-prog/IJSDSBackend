...
export const templates = {
  ...
  user_welcome: ({ name }) => ({
    ...
    html: layout("#4f46e5", "Welcome to IJSDS", `
      <p>Hi ${name},</p>
  ...
  author_welcome: ({ name }) => ({
    ...
    html: layout("#16a34a", "Author Onboarding", `
      <p>Hi ${name},</p>
  ...
  submission_received: ({ name, submissionId, title, submissionDate }) => ({
    ...
    html: layout("#7c3aed", "Submission Received", `
      <p>Hi ${name},</p>
  ...
  fee_information: ({ name, title }) => ({
    ...
    html: layout("#dc2626", "Publication Fees", `
      <p>Hi ${name},</p>
  ...
  review_assigned: ({ reviewerName, title, submissionId, deadline }) => ({
    ...
    html: layout("#0891b2", "Review Assignment", `
      <p>Hi ${reviewerName},</p>
  ...
  decision_made: ({ name, title, decision, decisionRationale }) => ({
    ...
    html: layout("#374151", "Editorial Decision", `
      <p>Hi ${name},</p>
  ...
  submission_accepted: ({ name, title }) => ({
    ...
    html: layout("#16a34a", "Submission Accepted", `
      <p>Hi ${name},</p>
  ...
  article_published: ({ name, title, doi, volume, issue, publicationDate }) => ({
    subject: `Article Published — ${title}`,
    html: layout("#4f46e5", "Article Published!", `
      <p>Hi ${name},</p>
      <p>Your article <strong>${title}</strong> has been officially published.</p>
      ${table([
        ["DOI", doi ? `<a href="https://doi.org/${doi}">${doi}</a>` : "Pending"],
        ["Volume", volume ?? "—"],
        ["Issue", issue ?? "—"],
  ...
  payment_confirmed: ({ name, paymentType, amount, articleTitle }) => ({
    ...
    html: layout("#16a34a", "Payment Confirmed", `
      <p>Hi ${name},</p>
  ...
  payment_received_editor: ({ editorName, articleTitle, userName, paymentType, amount }) => ({
    subject: `Payment Received — ${paymentType} for ${articleTitle}`,
    html: layout("#374151", "Payment Notification", `
      <p>Hi ${editorName},</p>
  ...
  payment_pending_editor: ({ editorName, articleTitle, userName, userEmail, paymentType }) => ({
    subject: `Payment Pending — ${paymentType} for ${articleTitle}`,
    html: layout("#f59e0b", "Payment Pending", `
      <p>Hi ${editorName},</p>
